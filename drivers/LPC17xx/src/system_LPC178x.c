...
#define __ECLK_DIV			  (EMCCLKSEL_Val & 0x01)
...
void SystemCoreClockUpdate (void)            /* Get Core Clock Frequency      */
{
  ...
  if ((LPC_SC->CCLKSEL &0x100) == 0) {            /* cclk = sysclk    */
    if ((LPC_SC->CLKSRCSEL & 0x01) == 0) {    /* sysclk = irc_clk */
          ...
          EMCClock        = (IRC_OSC / (LPC_SC->EMCCLKSEL & 0x1F));
    }
    else {                                        /* sysclk = osc_clk */
      if ((LPC_SC->SCS & 0x40) == 0) {
          ...
          EMCClock        = (OSC_CLK / (LPC_SC->EMCCLKSEL & 0x1F));
      }
    }
  }
  else {                                          /* cclk = pll_clk */
    if ((LPC_SC->PLL0STAT & 0x100) == 0) {        /* PLL0 not enabled */
      ...
      if ((LPC_SC->CLKSRCSEL & 0x01) == 0) {    /* sysclk = irc_clk */
          ...
          EMCClock        = (IRC_OSC * ((LPC_SC->PLL0STAT & 0x1F) + 1) / (LPC_SC->EMCCLKSEL & 0x1F));
      }
      else {                                        /* sysclk = osc_clk */
        if ((LPC_SC->SCS & 0x40) == 0) {
          SystemCoreClock = 0;                      /* this should never happen! */
          ...
          EMCClock        = (OSC_CLK * ((LPC_SC->PLL0STAT & 0x1F) + 1) / (LPC_SC->EMCCLKSEL & 0x1F));
        }
      }
    }
  }

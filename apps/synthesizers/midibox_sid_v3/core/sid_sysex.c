...
static s32 SID_SYSEX_ASID_Data(u8 midi_in);
...
static sid_sysex_asid_mode_t asid_mode;
...
s32 SID_SYSEX_Init(u32 mode)
{
  ...
  SID_SYSEX_ASID_ModeSet(SID_SYSEX_ASID_MODE_OFF);
...
s32 SID_SYSEX_Parser(mios32_midi_port_t port, u8 midi_in)
{
  ...
  if( asid_mode != SID_SYSEX_ASID_MODE_OFF && port != asid_last_sysex_port )
    return -1;
  ...
  switch( asid_state ) {
    ...
    case ASID_STATE_CMD:
      ...
      switch( asid_cmd ) {
        case 0x4c: // Start
	  SID_SYSEX_ASID_ModeSet(SID_SYSEX_ASID_MODE_ON);
	  break;
	  ...
	  if( asid_mode == SID_SYSEX_ASID_MODE_OFF )
	    SID_SYSEX_ASID_ModeSet(SID_SYSEX_ASID_MODE_ON);
	  asid_stream_ix = 0;
	  asid_reg_ix = 0;
	  break;
...
// ASID Mode Control
/////////////////////////////////////////////////////////////////////////////
sid_sysex_asid_mode_t SID_SYSEX_ASID_ModeGet(void)
{
  return asid_mode;
}

s32 SID_SYSEX_ASID_ModeSet(sid_sysex_asid_mode_t mode)
{
  asid_mode = mode;

...
# error "TODO!"
  ...
  MIOS32_SYS_LPC_FIODIR(SPEAKJET_PORT_RST_N, SPEAKJET_PIN_RST_N, 1); // output

  MIOS32_SYS_LPC_PINSEL(SPEAKJET_PORT_M0, SPEAKJET_PIN_M0, 0); // IO
  MIOS32_SYS_LPC_FIODIR(SPEAKJET_PORT_M0, SPEAKJET_PIN_M0, 1); // output

  MIOS32_SYS_LPC_PINSEL(SPEAKJET_PORT_M1, SPEAKJET_PIN_M1, 0); // IO
  MIOS32_SYS_LPC_FIODIR(SPEAKJET_PORT_M1, SPEAKJET_PIN_M1, 1); // output

  MIOS32_SYS_LPC_PINSEL(SPEAKJET_PORT_D2, SPEAKJET_PIN_D2, 0); // IO
  MIOS32_SYS_LPC_FIODIR(SPEAKJET_PORT_D2, SPEAKJET_PIN_D2, 0); // input
  MIOS32_SYS_LPC_PINMODE(SPEAKJET_PORT_D2, SPEAKJET_PIN_D2, 3); // activate pull-down (for the case that pin not connected)

  // enter demo mode
  MIOS32_SYS_LPC_FIOSET(SPEAKJET_PORT_M0, SPEAKJET_PIN_M0, 1);
  MIOS32_SYS_LPC_FIOSET(SPEAKJET_PORT_M1, SPEAKJET_PIN_M1, 0);

  // activate reset
  MIOS32_SYS_LPC_FIOSET(SPEAKJET_PORT_RST_N, SPEAKJET_PIN_RST_N, 0);
  MIOS32_DELAY_Wait_uS(1000);
  MIOS32_SYS_LPC_FIOSET(SPEAKJET_PORT_RST_N, SPEAKJET_PIN_RST_N, 1);
#else
...
# error "TODO!"
#elif defined(MIOS32_BOARD_MBHP_CORE_LPC17) || defined(MIOS32_BOARD_LPCXPRESSO)
  MIOS32_SYS_LPC_FIOSET(SPEAKJET_PORT_M0, SPEAKJET_PIN_M0, 0);
  MIOS32_SYS_LPC_FIOSET(SPEAKJET_PORT_M1, SPEAKJET_PIN_M1, 1);
#else
# error "this MIOS32_BOARD hasn't been prepared yet!"
#endif

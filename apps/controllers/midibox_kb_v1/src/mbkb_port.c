...
// has to be kept in synch with MBKB_PORT_InIxGet() !!!
static const mbkb_port_entry_t in_ports[MBKB_PORT_NUM_IN_PORTS] = {
  // port ID  Name
  { DEFAULT, "Def." },
  { USB0,    "USB1" },
  { USB1,    "USB2" },
...
// has to be kept in synch with MBKB_PORT_OutIxGet() !!!
static const mbkb_port_entry_t out_ports[MBKB_PORT_NUM_OUT_PORTS] = {
  // port ID  Name
  { DEFAULT, "Def." },
  { USB0,    "USB1" },
  { USB1,    "USB2" },
...
// has to be kept in synch with MBKB_PORT_ClkIxGet() !!!
static const mbkb_port_entry_t clk_ports[MBKB_PORT_NUM_CLK_PORTS] = {
  // port ID  Name
  { USB0,    "USB1" },
  { USB1,    "USB2" },
  { USB2,    "USB3" },
...
static u8 midi_out_ctr[MBKB_PORT_NUM_OUT_PORTS];
static mios32_midi_package_t midi_out_package[MBKB_PORT_NUM_OUT_PORTS];

static u8 midi_in_ctr[MBKB_PORT_NUM_IN_PORTS];
static mios32_midi_package_t midi_in_package[MBKB_PORT_NUM_IN_PORTS];

static mbkb_port_mon_filter_t mbkb_port_mon_filter;


/////////////////////////////////////////////////////////////////////////////
// Initialisation
/////////////////////////////////////////////////////////////////////////////
s32 MBKB_PORT_Init(u32 mode)
{
  int i;

  // init MIDI In/Out monitor variables
  for(i=0; i<MBKB_PORT_NUM_OUT_PORTS; ++i) {
    midi_out_ctr[i] = 0;
    midi_out_package[i].ALL = 0;
  }

  for(i=0; i<MBKB_PORT_NUM_IN_PORTS; ++i) {
    midi_in_ctr[i] = 0;
    midi_in_package[i].ALL = 0;
  }

  mbkb_port_mon_filter.ALL = 0;
  mbkb_port_mon_filter.MIDI_CLOCK = 1;
  mbkb_port_mon_filter.ACTIVE_SENSE = 1;
...
/////////////////////////////////////////////////////////////////////////////
s32 MBKB_PORT_InNumGet(void)
{
  return MBKB_PORT_NUM_IN_PORTS;
}

s32 MBKB_PORT_OutNumGet(void)
{
  return MBKB_PORT_NUM_OUT_PORTS;
}

s32 MBKB_PORT_ClkNumGet(void)
{
  return MBKB_PORT_NUM_CLK_PORTS;
}


/////////////////////////////////////////////////////////////////////////////
// Returns name of MIDI IN/OUT port in (4 characters + zero terminator)
// port_ix is within 0..MBKB_PORT_In/OutNumGet()-1
/////////////////////////////////////////////////////////////////////////////
char *MBKB_PORT_InNameGet(u8 port_ix)
{
  if( port_ix >= MBKB_PORT_NUM_IN_PORTS )
    return "----";
  else
    return (char *)in_ports[port_ix].name;
}

char *MBKB_PORT_OutNameGet(u8 port_ix)
{
  if( port_ix >= MBKB_PORT_NUM_OUT_PORTS )
    return "----";
  else
    return (char *)out_ports[port_ix].name;
}

char *MBKB_PORT_ClkNameGet(u8 port_ix)
{
  if( port_ix >= MBKB_PORT_NUM_CLK_PORTS )
    return "----";
  else
...
// port_ix is within 0..MBKB_PORT_In/OutNumGet()-1
/////////////////////////////////////////////////////////////////////////////
mios32_midi_port_t MBKB_PORT_InPortGet(u8 port_ix)
{
  if( port_ix >= MBKB_PORT_NUM_IN_PORTS )
...
mios32_midi_port_t MBKB_PORT_OutPortGet(u8 port_ix)
{
  if( port_ix >= MBKB_PORT_NUM_OUT_PORTS )
    return 0xff; // dummy interface
  else
...
mios32_midi_port_t MBKB_PORT_ClkPortGet(u8 port_ix)
{
  if( port_ix >= MBKB_PORT_NUM_CLK_PORTS )
    return 0xff; // dummy interface
  else
...
/////////////////////////////////////////////////////////////////////////////
u8 MBKB_PORT_InIxGet(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBKB_PORT_NUM_IN_PORTS; ++ix) {
...
u8 MBKB_PORT_OutIxGet(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBKB_PORT_NUM_OUT_PORTS; ++ix) {
...
u8 MBKB_PORT_ClkIxGet(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBKB_PORT_NUM_CLK_PORTS; ++ix) {
...
/////////////////////////////////////////////////////////////////////////////
s32 MBKB_PORT_InCheckAvailable(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBKB_PORT_NUM_IN_PORTS; ++ix) {
    if( in_ports[ix].port == port ) {
...
s32 MBKB_PORT_OutCheckAvailable(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBKB_PORT_NUM_OUT_PORTS; ++ix) {
    if( out_ports[ix].port == port ) {
...
s32 MBKB_PORT_ClkCheckAvailable(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBKB_PORT_NUM_CLK_PORTS; ++ix) {
    if( clk_ports[ix].port == port ) {
...
/////////////////////////////////////////////////////////////////////////////
mios32_midi_package_t MBKB_PORT_OutPackageGet(mios32_midi_port_t port)
{
  u8 port_ix = 0;
  ...
  if( port != DEFAULT ) {
    port_ix = MBKB_PORT_OutIxGet(port);
...
/////////////////////////////////////////////////////////////////////////////
mios32_midi_package_t MBKB_PORT_InPackageGet(mios32_midi_port_t port)
{
  u8 port_ix = 0;
  mios32_midi_package_t empty;
  empty.ALL = 0;
  ...
  if( port != DEFAULT ) {
    port_ix = MBKB_PORT_InIxGet(port);
...
/////////////////////////////////////////////////////////////////////////////
s32 MBKB_PORT_MonFilterSet(mbkb_port_mon_filter_t filter)
{
  mbkb_port_mon_filter = filter;
  return 0; // no error
}

mbkb_port_mon_filter_t MBKB_PORT_MonFilterGet(void)
{
  return mbkb_port_mon_filter;
}
...
/////////////////////////////////////////////////////////////////////////////
s32 MBKB_PORT_NotifyMIDITx(mios32_midi_port_t port, mios32_midi_package_t package)
{
  // MIDI Out monitor function
  u8 mon_filtered = 0;
  if( mbkb_port_mon_filter.MIDI_CLOCK && package.evnt0 == 0xf8 )
    mon_filtered = 1;
  else if( mbkb_port_mon_filter.ACTIVE_SENSE && package.evnt0 == 0xfe )
    mon_filtered = 1;
...
/////////////////////////////////////////////////////////////////////////////
s32 MBKB_PORT_NotifyMIDIRx(mios32_midi_port_t port, mios32_midi_package_t package)
{
...
/////////////////////////////////////////////////////////////////////////////
s32 MBKB_PORT_EventNameGet(mios32_midi_package_t package, char *label, u8 num_chars)
{

...
// has to be kept in synch with MBCV_PORT_InIxGet() !!!
static const mbcv_port_entry_t in_ports[MBCV_PORT_NUM_IN_PORTS] = {
  // port ID  Name
  { DEFAULT, "Def." },
  { USB0,    "USB1" },
  { UART0,   "IN1 " },
...
// has to be kept in synch with MBCV_PORT_OutIxGet() !!!
static const mbcv_port_entry_t out_ports[MBCV_PORT_NUM_OUT_PORTS] = {
  // port ID  Name
  { DEFAULT, "Def." },
  { USB0,    "USB1" },
  { UART0,   "OUT1" },
...
// has to be kept in synch with MBCV_PORT_ClkIxGet() !!!
static const mbcv_port_entry_t clk_ports[MBCV_PORT_NUM_CLK_PORTS] = {
  // port ID  Name
  { USB0,    "USB1" },
  { UART0,   "MID1" },
  { UART1,   "MID2" },
...
static u8 midi_out_ctr[MBCV_PORT_NUM_OUT_PORTS];
static mios32_midi_package_t midi_out_package[MBCV_PORT_NUM_OUT_PORTS];

static u8 midi_in_ctr[MBCV_PORT_NUM_IN_PORTS];
static mios32_midi_package_t midi_in_package[MBCV_PORT_NUM_IN_PORTS];

static mbcv_port_mon_filter_t mbcv_port_mon_filter;


/////////////////////////////////////////////////////////////////////////////
// Initialisation
/////////////////////////////////////////////////////////////////////////////
s32 MBCV_PORT_Init(u32 mode)
{
  int i;

  // init MIDI In/Out monitor variables
  for(i=0; i<MBCV_PORT_NUM_OUT_PORTS; ++i) {
    midi_out_ctr[i] = 0;
    midi_out_package[i].ALL = 0;
  }

  for(i=0; i<MBCV_PORT_NUM_IN_PORTS; ++i) {
    midi_in_ctr[i] = 0;
    midi_in_package[i].ALL = 0;
  }

  mbcv_port_mon_filter.ALL = 0;
  mbcv_port_mon_filter.MIDI_CLOCK = 1;
  mbcv_port_mon_filter.ACTIVE_SENSE = 1;
...
/////////////////////////////////////////////////////////////////////////////
s32 MBCV_PORT_InNumGet(void)
{
  return MBCV_PORT_NUM_IN_PORTS;
}

s32 MBCV_PORT_OutNumGet(void)
{
  return MBCV_PORT_NUM_OUT_PORTS;
}

s32 MBCV_PORT_ClkNumGet(void)
{
  return MBCV_PORT_NUM_CLK_PORTS;
}


/////////////////////////////////////////////////////////////////////////////
// Returns name of MIDI IN/OUT port in (4 characters + zero terminator)
// port_ix is within 0..MBCV_PORT_In/OutNumGet()-1
/////////////////////////////////////////////////////////////////////////////
char *MBCV_PORT_InNameGet(u8 port_ix)
{
  if( port_ix >= MBCV_PORT_NUM_IN_PORTS )
    return "----";
  else
    return (char *)in_ports[port_ix].name;
}

char *MBCV_PORT_OutNameGet(u8 port_ix)
{
  if( port_ix >= MBCV_PORT_NUM_OUT_PORTS )
    return "----";
  else
    return (char *)out_ports[port_ix].name;
}

char *MBCV_PORT_ClkNameGet(u8 port_ix)
{
  if( port_ix >= MBCV_PORT_NUM_CLK_PORTS )
    return "----";
  else
...
// port_ix is within 0..MBCV_PORT_In/OutNumGet()-1
/////////////////////////////////////////////////////////////////////////////
mios32_midi_port_t MBCV_PORT_InPortGet(u8 port_ix)
{
  if( port_ix >= MBCV_PORT_NUM_IN_PORTS )
...
mios32_midi_port_t MBCV_PORT_OutPortGet(u8 port_ix)
{
  if( port_ix >= MBCV_PORT_NUM_OUT_PORTS )
    return 0xff; // dummy interface
  else
...
mios32_midi_port_t MBCV_PORT_ClkPortGet(u8 port_ix)
{
  if( port_ix >= MBCV_PORT_NUM_CLK_PORTS )
    return 0xff; // dummy interface
  else
...
/////////////////////////////////////////////////////////////////////////////
u8 MBCV_PORT_InIxGet(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBCV_PORT_NUM_IN_PORTS; ++ix) {
...
u8 MBCV_PORT_OutIxGet(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBCV_PORT_NUM_OUT_PORTS; ++ix) {
...
u8 MBCV_PORT_ClkIxGet(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBCV_PORT_NUM_CLK_PORTS; ++ix) {
...
/////////////////////////////////////////////////////////////////////////////
s32 MBCV_PORT_InCheckAvailable(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBCV_PORT_NUM_IN_PORTS; ++ix) {
    if( in_ports[ix].port == port ) {
...
s32 MBCV_PORT_OutCheckAvailable(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBCV_PORT_NUM_OUT_PORTS; ++ix) {
    if( out_ports[ix].port == port ) {
...
s32 MBCV_PORT_ClkCheckAvailable(mios32_midi_port_t port)
{
  u8 ix;
  for(ix=0; ix<MBCV_PORT_NUM_CLK_PORTS; ++ix) {
    if( clk_ports[ix].port == port ) {
...
/////////////////////////////////////////////////////////////////////////////
mios32_midi_package_t MBCV_PORT_OutPackageGet(mios32_midi_port_t port)
{
  u8 port_ix = 0;
...
/////////////////////////////////////////////////////////////////////////////
mios32_midi_package_t MBCV_PORT_InPackageGet(mios32_midi_port_t port)
{
  u8 port_ix = 0;
  mios32_midi_package_t empty;
  empty.ALL = 0;
...
/////////////////////////////////////////////////////////////////////////////
s32 MBCV_PORT_MonFilterSet(mbcv_port_mon_filter_t filter)
{
  mbcv_port_mon_filter = filter;
  return 0; // no error
}

mbcv_port_mon_filter_t MBCV_PORT_MonFilterGet(void)
{
  return mbcv_port_mon_filter;
}
...
/////////////////////////////////////////////////////////////////////////////
s32 MBCV_PORT_NotifyMIDITx(mios32_midi_port_t port, mios32_midi_package_t package)
{
  // MIDI Out monitor function
  u8 mon_filtered = 0;
  if( mbcv_port_mon_filter.MIDI_CLOCK && package.evnt0 == 0xf8 )
    mon_filtered = 1;
  else if( mbcv_port_mon_filter.ACTIVE_SENSE && package.evnt0 == 0xfe )
    mon_filtered = 1;
...
/////////////////////////////////////////////////////////////////////////////
s32 MBCV_PORT_NotifyMIDIRx(mios32_midi_port_t port, mios32_midi_package_t package)
{
...
/////////////////////////////////////////////////////////////////////////////
s32 MBCV_PORT_EventNameGet(mios32_midi_package_t package, char *label, u8 num_chars)
{

...
s32 MID_PARSER_Read(void)
{
   ...
   while (!MID_FILE_eof())
   {
      file_pos += MID_FILE_read(chunk_type, 4);
...
s32 MID_PARSER_FetchEvents(u32 tick_offset, u32 num_ticks)
{
   ...
   for(track=0; track<midi_tracks_num; ++mt, ++track)
   {
      while (mt->file_pos < mt->chunk_end)
      {
         ...
         // set file pos
         MID_FILE_seek(mt->file_pos);

         // get event
         u8 event;
         mt->file_pos += MID_FILE_read(&event, 1);
         ...
         if (event == 0xf0)
         {
            ...
            {
               u8 evnt0;
               mt->file_pos += MID_FILE_read(&evnt0, 1);
         ...
         else if( event == 0xff )
         {
            // Meta Event
            u8 meta;
            mt->file_pos += MID_FILE_read(&meta, 1);
            u32 length = (u32)MID_PARSER_ReadVarLen(&mt->file_pos);
            ...
            if( mid_parser_playmeta_callback != NULL )
            {
               ...
               if (buflen)
               {
                  ...
                  mt->file_pos += MID_FILE_read(meta_buffer, buflen);
                  ...
                  if (length > buflen)
                  {
                     ...
                     for(i=buflen; i<length; ++i)
                        mt->file_pos += MID_FILE_read(&dummy, 1);
                  }
               }
         ...
         else
         {
            ...
            if( event & 0x80 )
            {
               ...
               u8 evnt1;
               mt->file_pos += MID_FILE_read(&evnt1, 1);
            ...
            case PitchBend:
               {
                  u8 evnt2;
                  mt->file_pos += MID_FILE_read(&evnt2, 1);

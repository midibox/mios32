...
s32 SEQ_CORE_Handler(void)
{
  ...
  do {
    ...
    if( SEQ_BPM_ChkReqSongPos(&new_song_pos) ) {
      // release slave mute
      seq_core_slaveclk_mute = SEQ_CORE_SLAVECLK_MUTE_Off;
	DEBUG_MSG("SEQ_BPM_ChkReqSongPos uit core");
...
s32 SEQ_CORE_Tick(u32 bpm_tick, s8 export_track, u8 mute_nonloopback_tracks)
{
  ...
  for(round=0; round<2; ++round) {
	...
	for(track=first_track; track<(SEQ_CORE_NUM_TRACKS_PER_GROUP+first_track); ++t, ++tcc, ++track) {
...
//s32 SEQ_CORE_BPM_Update(int bpm, int sweep_ramp)
{
  if( sweep_ramp <= 0.0 ) {

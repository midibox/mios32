	...
	return -1; // not mapped
      ...
      switch( encoder ) {
       // case SEQ_UI_ENCODER_GP1:
		//	if( SEQ_UI_Var8_Inc(&ui_selected_item, 0, 6, incrementer) >= 1 )
		//		return 1;
		//	else
		//		return 0;
		//	break;
...
static s32 LCD_Handler(u8 high_prio)
{
  ...
  case PR_DIALOG_EDIT_LABEL: {
	...
	if( ui_cursor_flash ) {
	  ...
	  SEQ_LCD_CursorSet(1 + ui_edit_name_cursor, 2);
	  SEQ_LCD_PrintChar('*');
	}
      } else {
	if( ui_edit_name_cursor < 5 ) {
	  SEQ_LCD_PrintString("Please enter Track  ");
	  SEQ_LCD_CursorSet(0, 1);
	  SEQ_LCD_PrintString("Category for        ");
	  SEQ_LCD_PrintGxTy(ui_selected_group, ui_selected_tracks);
	...
	*/
	...
	if  ((9) == ui_selected_item) {
		SEQ_LCD_PrintString("VelN<VelA PRE-        ");
	} else if ((10) == ui_selected_item) {
		SEQ_LCD_PrintString("VelN>VelA<PRE-        ");
	} else  {
		SEQ_LCD_PrintString("VelN VelA PRE-        ");

...
if( ui_page == SEQ_UI_PAGE_SONG ) {
	...
	if (seq_hwcfg_mb909.enabled && seq_hwcfg_mb909.multimachine==0){
		seq_core_trk_muted = ~((1<< track_button) & 0x0f);
		seq_song_step_t s = SEQ_SONG_StepEntryGet(ui_song_edit_pos);
		s.bank_g2= track_button;
...
s32 SEQ_UI_LCD_Handler(void)
{
  ...
  if( !SEQ_FILE_HW_ConfigLocked() ) {
	...
	if( boot_animation_lcd_pos == 0 ) {
      ...
      //MIOS32_LCD_GCursorSet( 12, 18);
	  SEQ_LCD_PrintString("Search ");
	  //MIOS32_LCD_GCursorSet( 10, 34);
    ...
    if( seq_ui_backup_req )
      MIOS32_LCD_PrintString("Copy Files!!!");
    else if( seq_ui_format_req )
      MIOS32_LCD_PrintString("Creating Files!!!");
    else
      MIOS32_LCD_PrintString("Don't know what I'm doing! :-/");

    if( seq_file_backup_notification != NULL ) {
      int i;
      MIOS32_LCD_CursorSet(0, 1);
      MIOS32_LCD_PrintFormattedString("Creating %s", seq_file_backup_notification);

      MIOS32_LCD_CursorSet(0, 2);
      MIOS32_LCD_PrintString("Total: [");
      for(i=0; i<20; ++i)
	MIOS32_LCD_PrintChar((i>(seq_file_backup_percentage/5)) ? ' ' : '#');
      MIOS32_LCD_PrintFormattedString("] %3d%%", seq_file_backup_percentage);

      MIOS32_LCD_CursorSet(0, 3);
      MIOS32_LCD_PrintString("File:  [");
      for(i=0; i<20; ++i)
	MIOS32_LCD_PrintChar((i>(file_copy_percentage/5)) ? ' ' : '#');
      MIOS32_LCD_PrintFormattedString("] %3d%%", file_copy_percentage);
    }
...
s32 SEQ_UI_LCD_Update(void)
{
  ...
  if( ui_msg_ctr ) {
	...
	//MIOS32_LCD_CursorSet(msg_x, line+5);
  ...
  if( (TASK_MSD_EnableGet() > 0) && ui_page != SEQ_UI_PAGE_DISK ) {
    MIOS32_LCD_CursorSet(21-11, 0);
    if( ui_cursor_flash ) SEQ_LCD_PrintSpaces(13); else MIOS32_LCD_PrintString(" [MSD USB] ");
    MIOS32_LCD_CursorSet(21-11, 1);
    char str[5];

...
{
  //DEBUG_MSG("SEQ_PATTERN_Change gestart uit pattern");
  if( group >= SEQ_CORE_NUM_GROUPS )
  ...
  if( force_immediate_change || !SEQ_BPM_IsRunning() || SEQ_BPM_TickGet() == 0 || ui_seq_pause ) {
	 //DEBUG_MSG("SEQ_PATTERN_Change force_immediate_change");
	...
	//DEBUG_MSG("SEQ_PATTERN_Change geen force_immediate_change maar request");
    // else request change
    portENTER_CRITICAL();
    pattern.REQ = 1;
    seq_pattern_req[group] = pattern;
	...
	//DEBUG_MSG("SEQ_PATTERN_Change SEQ_TASK_PatternResume");

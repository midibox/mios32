...
long JUCE_MIDI_Init(long mode)
{
	...
	MIDIOut = contentComponent_p->midiDeviceManager.getDefaultMidiOutput();
	
	// if it's failed, try to just get the system default
	if (MIDIOut == NULL)
	{
		// this makes a new port on linux... f*** knows what it does elsewhere. Should just choose the default

...
#if 1
      // extra MBSEQV4L: don't send CC if assigned to 0 (not recorded yet)
      if( !tcc->lay_const[1*16 + par_layer] )
	break;
#endif      
...
s32 SEQ_LAYER_CopyPreset(u8 track, u8 only_layers, u8 all_triggers_cleared, u8 init_assignments)
{
  ...
  if( !only_layers ) {
    ...
    if( init_assignments ) {
      switch( event_mode ) {
        case SEQ_EVENT_MODE_Note__Disabled:
        case SEQ_EVENT_MODE_Chord__Disabled:
	  ...
	  if( event_mode == SEQ_EVENT_MODE_Combined ) {
	    ...
	    SEQ_CC_Set(track, SEQ_CC_LAY_CONST_A1, (event_mode == SEQ_EVENT_MODE_Chord__Disabled) ? SEQ_PAR_Type_Chord : SEQ_PAR_Type_Note);

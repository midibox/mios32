...
{
#if 1
  // extra for MBSEQ V4L: seq_record_armed_tracks and auto-assignment
  if( !seq_record_armed_tracks )
  ...
  if( (midi_package.event == NoteOn) || (midi_package.event == NoteOff) ) {
	...
	if( *layer_type_ptr == SEQ_PAR_Type_CC &&
	  ...
	  if( !(seq_record_armed_tracks & (1 << track)) )
	    return 0;

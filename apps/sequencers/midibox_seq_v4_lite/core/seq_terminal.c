...
s32 SEQ_TERMINAL_PrintSdCardInfo(void *_output_function)
{
...

  {
    ...
    for(bank=0; bank<SEQ_FILE_B_NUM_BANKS; ++bank) {
      ...
      if( num_patterns )
	out("File /SESSIONS/%s/MBSEQ_B%d.V4T: valid (%d patterns)\n", seq_file_session_name, bank+1, num_patterns);
      else
	out("File /SESSIONS/%s/MBSEQ_B%d.V4T: doesn't exist\n", seq_file_session_name, bank+1, num_patterns);
    }

    if( SEQ_FILE_G_Valid() )
      out("File /SESSIONS/%s/MBSEQ_G.V4T: valid\n", seq_file_session_name);
    else
      out("File /SESSIONS/%s/MBSEQ_G.V4T: doesn't exist\n", seq_file_session_name);
    
    if( SEQ_FILE_HW_Valid() )
      out("File /SESSIONS/%s/MBSEQ_HW.V4T: valid\n", seq_file_session_name);
    else
      out("File /SESSIONS/%s/MBSEQ_HW.V4T: doesn't exist or hasn't been re-loaded\n", seq_file_session_name);
  }

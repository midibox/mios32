// $Id$
/*
 * MIDI file Sequencer w/ SD card support
...
#else
	// print play time in minutes/seconds
	float tpqn = 60.0 / SEQ_BPM_Get(); // time per quarter note
	float seconds = (tpqn / (float)SEQ_BPM_PPQN_Get()) * SEQ_BPM_TickGet();
	u32 seconds_u = (u32)seconds;
	MIOS32_LCD_CursorSet(0, 0);
	MIOS32_LCD_PrintFormattedString("%3d:%02d ", seconds_u / 60, seconds_u % 60);

	if( seq_pause ) {
	  MIOS32_LCD_PrintString("Pause");
	} else if( SEQ_BPM_IsRunning() ) {
	  MIOS32_LCD_PrintString("Play ");
	} else {
	  MIOS32_LCD_PrintString("Stop ");
	}

	MIOS32_LCD_PrintString(SEQ_BPM_IsMaster() ? " Mst" : " Slv");
#endif
        MIOS32_LCD_CursorSet(0, 1);
	MIOS32_LCD_PrintFormattedString("%-12s    ", MID_FILE_NameGet());
...
void APP_DIN_NotifyToggle(u32 pin, u32 pin_value)
{
  ...
  switch( pin ) {
    ...
    case DIN_NUMBER_SNAPSHOT: // used as "Next file" button
      SEQ_PlayFileReq(1);
      break;
  }
}

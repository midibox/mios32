...
s32 SEQ_CORE_Init(u32 mode)
{
  ...
  seq_core_glb_mb909_multimachine= 0;
  seq_core_glb_mb909_non909= 0; 
  seq_core_glb_mb909_expert=1;
...
s32 SEQ_CORE_Tick(u32 bpm_tick, s8 export_track, u8 mute_nonloopback_tracks)
{
  ...
  for(round=0; round<2; ++round) {
    ...
    u8 first_track;
	u8 last_track;
	if ((seq_hwcfg_mb909.enabled && seq_core_glb_mb909_multimachine==1 && seq_core_glb_mb909_non909==1) || SEQ_SONG_ActiveGet() ){
		first_track= 0;
		last_track=12;		
	} else{
		first_track= ui_selected_group*4;
		last_track= ui_selected_group*4;
	}
	 
	for(track=first_track; track<(SEQ_CORE_NUM_TRACKS_PER_GROUP+last_track); ++t, ++tcc, ++track) {
	*/
